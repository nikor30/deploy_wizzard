...
function pnpStateBadge(state: string | null) {
...
function SelectView({ onJobCreated }: { onJobCreated: (job: Job) => void }) {
  ...
  return (
    <div className="mt-8">
      ...
      {(devices ?? []).some((d) => d.state && d.state !== 'Unclaimed') && (
        <p className="mt-3 rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-800 dark:bg-amber-950/40 dark:text-amber-300">
          Devices in <strong>Error</strong>, <strong>Planned</strong> or <strong>Onboarding</strong>{' '}
          are earlier onboarding attempts still held by Catalyst Center. After a factory reset they
          can be claimed again — pick them here just like an unclaimed device. If a stale record
          blocks a re-claim, delete it in Catalyst Center first.
        </p>
      )}
      <ErrorBanner message={error} />
            ...
            <tbody>
              {filtered.map((device) => (
                ...
                >
                  ...
                  <td className="px-3 py-2">{pnpStateBadge(device.state)}</td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-3 py-6 text-center text-slate-400">
                    No onboarding devices found in Catalyst Center.
                  </td>
                </tr>
              )}
            </tbody>
